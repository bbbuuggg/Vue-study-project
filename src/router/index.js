...
/* Layout 这里的 Layout 是整个项目的布局组件，
表示页面的整体框架。所有路由基本上都使用这个布局作为父组件，
页面内容则作为子路由显示在布局的 router-view 中。*/
import Layout from '@/layout'

/* Router Modules 模块化的路由配置*/
...
/**
 * 动态加载路由，即根据用户角色动态加载，不同用户角色看到不同页面
 * asyncRoutes
...
export const asyncRoutes = [
   ...
   * 模块化路由
   * 如果要调整这些模块的权限调整router\modules下的js文件在meta中加角色控制就好了
   * **/
  componentsRouter,
  chartsRouter,
  nestedRouter,
...
// 创建路由实例，以便在需要时重置路由（比如权限变化时）
const createRouter = () => new Router({
  // mode: 'history', // require service support
  scrollBehavior: () => ({ y: 0 }), // 每次切换时路由滚动到顶部
  routes: constantRoutes // 初始路由只加载constantRoutes
})
// 路由实例
